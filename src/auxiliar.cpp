...
Matrix4d inverse_se3(Matrix4d T){
    Matrix4d Tinv = Matrix4d::Identity();  //初始化为单位矩阵
    Matrix3d R;  //旋转矩阵
    Vector3d t;//平移矩阵
    t = T.block(0,3,3,1);
    R = T.block(0,0,3,3);
...
//向量的辅助函数和结构
//和标准差相比，标准差计算的是每个数据到均值的距离平方，因此很容易受到个别偏差较大数据的影响。而绝对中位差因为统计的中位数，少量的异常值通常不会影响最终的结果，因此是鲁棒的
...
void vector_mean_stdv_mad( vector<double> residues, double &mean, double &stdv )
{
    ...
    if( residues.size() != 0 )//如果样本容器不为空
    {
        // Return the standard deviation of vector with MAD estimation
        //使用MAD估计返回向量的标准偏差  绝对中位数MAD是对单变量数值型数据的样本偏差的一种鲁棒性测量。对于单变量数据集 ,MAD的定义为数据点到中位数的绝对偏差的中位数。
        int n_samples = residues.size();//获取样本数量
        vector<double> residues_ = residues; //复制一份样本副本
        sort( residues.begin(),residues.end() ); //将样本按从大到小重新排列
        double median = residues[ n_samples/2 ];  //获取样本的中位数
        //遍历每一份样本 计算当前样本与样本中位数的差值
        for( int i = 0; i < n_samples; i++)
            //residues[i] = fabsf( residues[i] - median );  // fabsf（）处理float类型的取绝对值(功能可以被double覆盖)
            residues[i] = fabs( residues[i] - median );  //TODO  修改过源码是上面  数据到中位数的绝对偏差
        sort( residues.begin(),residues.end() );  //再将与样本中位数的差值进行排序
        stdv = 1.4826 * residues[ n_samples/2 ];  //用1.4826*样本中位数差值的中位数，得到stdv
        ...
        //遍历样本
        for( int i = 0; i < n_samples; i++)
            if( residues_[i] < 2.0 * stdv )  //如果样本数据小于2倍的stdv 说明是正常值 计算该样本数据的累加和
            {
                mean += residues_[i]; //
                k++;  //有效样本数+1
            }
        ...
        if( k >= int( 0.2 * residues.size()) )  //如果有效样本数大于等于0.2倍的样本数
            mean /= double(k); //将有效样本数求平均
        else
        {
            k = 0;
            mean = 0.f;
...
double vector_stdv_mad( vector<double> residues)
{
    if( residues.size() != 0 )
    {
        ...
        //使用MAD估计返回向量的标准偏差
        int n_samples = residues.size();
        sort( residues.begin(),residues.end() );//将投影误差按从小到大排列
        double median = residues[ n_samples/2 ]; //得到投影误差的中位数
        for( int i = 0; i < n_samples; i++)
