...
Config::Config()
{
    ...
    // kf decision (SLAM) parameters  决定是否是关键帧的参数
    ...
    use_lsd_lines      = true;     // true if using LSD detector
    use_fld_lines      = false;     // true if using FLD detector
    use_EDlines        = false;     // true if using EDLines detector
    ...
    // Adaptative FAST parameters    自适应FAST参数
    ...
    orb_fast_min_th   = 8;         // min FAST threshold
    // LSD parameters
...
{
    //调用默认构造函数
    static Config instance; // Instantiated on first use and guaranteed to be destroyed 第一次使用时实例化并保证销毁
    return instance;
}
...
{
    //boost::filesystem::exists（）判断该文件是否存在 boost::filesystem::is_regular（）判断文件是否是普通文件
    if (!boost::filesystem::exists(config_file) || !boost::filesystem::is_regular(config_file)) {
        ...
        return;  //如果文件不存在或者不是普通文件，输出错误提示信息并返回
    }

    //C++使用yaml-cpp解析yaml文件
    YAML::Node config = YAML::LoadFile(config_file);//下载并解析配置文件
    ...
    Config::useTexture() = loadSafe(config, "use_texture", Config::useTexture());
    Config::fuseLineth() = loadSafe(config, "fuse_line_th", Config::fuseLineth());
    
    Config::useLSDLines() = loadSafe(config, "use_lsd_lines", Config::useLSDLines());
    Config::useFLDLines() = loadSafe(config, "use_fld_lines", Config::useFLDLines());
    Config::useEDLines() = loadSafe(config, "use_EDlines", Config::useEDLines());
    Config::useMyORB() = loadSafe(config, "use_my_orb", Config::useMyORB());
    
    Config::useED_min_angle() = loadSafe(config, "ED_min_angle", Config::useED_min_angle());
    Config::useED_min_distance() = loadSafe(config, "ED_min_distance", Config::useED_min_distance());
    Config::useED_min_dis() = loadSafe(config, "ED_min_dis", Config::useED_min_dis());
    ...
    Config::orbFastminTh() = loadSafe(config, "orb_fast_min_th", Config::orbFastminTh());
