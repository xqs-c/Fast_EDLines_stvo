...
public:
    ...
    static bool&    useTexture()         { return getInstance().use_texture; }
    static int&     fuseLineth()        { return getInstance().fuse_line_th; } //TODO 提取线特征的场景纹理丰富度阈值
    
    static bool&    useLSDLines()       { return getInstance().use_lsd_lines; } //TODO 使用LSD
    static bool&    useFLDLines()       { return getInstance().use_fld_lines; }
    static bool&    useEDLines()        { return getInstance().use_EDlines; } //TODO 使用EDLines
    
    static bool&    useMyORB()          { return getInstance().use_my_orb; } //TODO 使用自己的ORB提取器
    
    static double&  useED_min_angle()      { return getInstance().ED_min_angle; } //TODO EDLines算法中进行线段融合的角度阈值
    static double&  useED_min_distance()   { return getInstance().ED_min_distance; }
    static int&     useED_min_dis()        { return getInstance().ED_min_dis; }
    ...
    static int&     orbFastminTh()      { return getInstance().orb_fast_min_th; } //TODO FAST角点的最小阈值
    ...
    bool use_texture; //TODO 是否采取场景纹理信息丰富度线特征提取策略
    
    bool lr_in_parallel;
    bool pl_in_parallel;
    bool best_lr_matches;
    bool adaptative_fast;
    bool use_lsd_lines; //TODO 使用LSD
    bool use_fld_lines;
    bool use_EDlines; //TODO 使用EDLines
    
    int fuse_line_th;//TODO 提取线特征的场景纹理丰富度阈值
    
    bool use_my_orb; //TODO  使用自己的ORB提取器标志位
    
    double ED_min_angle; //EDLines算法中进行线段融合的角度阈值
    double ED_min_distance; //EDLines算法中进行线段融合的端点距离阈值
    int ED_min_dis; //EDLines算法中进行线段融合的水平或垂直距离阈值
    ...
    int    orb_patch_size;
    int    orb_fast_th;
    int    orb_fast_min_th; //TODO FAST角点的最小阈值
